...
- **游戏分支**
  - 1.18.4: release/1.18.4
  - 1.19.0: release/1.19.0

- **游戏提交**
  - 1.18.4: 6f57f9eb07db28b40185274899544bb29b3e579f
  - 1.19.0: 2753ca4000401afbe17ad1c6cfeeb0acd0d927c1

- **引擎分支**
  - 1.18.4: titus/release/1.18.2
  - 1.19.0: titus/release/1.19.0

- **引擎提交**
  - 1.18.4: 72b717fb89c6be4439541b07c480f8d6ec050f0d
  - 1.19.0: f22559721a07e0156b809de8da35ffdc78a47d15

---

## 一、整体差异统计总览

- **新增文件**: 583 个（仅存在于 1.19.0 的文件）
- **删除文件**: 207 个（仅存在于 1.18.4 的文件）
- **修改文件**: 3,789 个（两版本都存在但内容/大小不同）
- **未变化文件**: 45,238 个（两版本完全相同）
...
- **变更类型：教条存储**
  - 1.18.4: 嵌套在组 wrapper 中
  - 1.19.0: 扁平独立定义
  - 玩家影响: 更清晰的模块化

- **变更类型：教条分组**
  - 1.18.4: 内嵌在文件结构中
  - 1.19.0: doctrine_group_types 显式管理
  - 玩家影响: 便于批量修改

- **变更类型：圣地定义**
  - 1.18.4: 内嵌在教条/宗教中
  - 1.19.0: 独立的 holy_site_types
  - 玩家影响: 圣地系统独立化
...
- **变更类型：家族定义**
  - 1.18.4: 无
  - 1.19.0: rf_abrahamic/eastern/sinitic/pagan
  - 玩家影响: 统一家族系统
...
- **扁平图标（flat_icons）**: 38+ 个，每个 14.19 KB
- **骑士徽章图标**: 48+ 个，每个 11.39-45.12 KB
- **DLC 图标**: 2 个，每个 56.38 KB
- **故事循环图标**: 4 个，每个 56.38-64.12 KB
...
- **英语（english）**: 12 个文件 - ledger, court_visuals, debug_story, elder_events, knight_permissions
- **法语（french）**: 12 个文件 - 同上
- **德语（german）**: 12 个文件 - 同上
- **日语（japanese）**: 12 个文件 - 同上
- **韩语（korean）**: 12 个文件 - 同上
- **波兰语（polish）**: 12 个文件 - 同上
- **俄语（russian）**: 12 个文件 - 同上
- **简体中文（simp_chinese）**: 12 个文件 - 同上
- **西班牙语（spanish）**: 12 个文件 - 同上
- **总计**: 108 个文件
...
- **binaries/**: 新增 2 个，删除 1 个，修改 ~5 个
- **clausewitz/**: 新增 10 个，删除 0 个，修改 ~20 个
- **game/common/religion/**: 新增 +100 个，删除 -50 个，修改 ~500 个
- **game/events/**: 新增 +15 个，删除 ~5 个，修改 ~200 个
- **game/gui/**: 新增 +25 个，删除 ~3 个，修改 ~150 个
- **game/gfx/**: 新增 +200 个，删除 ~30 个，修改 ~800 个
- **game/localization/**: 新增 +120 个，删除 ~10 个，修改 ~600 个
- **jomini/**: 新增 +30 个，删除 0 个，修改 ~100 个
- **launcher/**: 新增 1 个，删除 0 个，修改 ~5 个
- **总计**: 新增 583 个，删除 207 个，修改 3,789 个
